...
for i in range(6):
    ...
    dataset_data[0].to_csv("snp_datasets/data_no_snp_" + str(snp_ind), sep=';', index=False)
    dataset_data[1].to_csv("snp_datasets/data_no_snp_" + str(snp_ind) + "_response", sep=';', index=False)
...
for season in DATASET_SEASONS.keys():
    ...
    dataset_data[0].to_csv("seasons_datasets/data_season_" + season, sep=';', index=False)
    dataset_data[1].to_csv("seasons_datasets/data_season_" + season + "_response", sep=';', index=False)

# creating datasets per geo_id

for geo_id in [0., 2., 3.]:
    dataset_data = parse_per_key(merged_dataset, y_merged, geo_id, "geo_id")
    dataset_data[0].to_csv("geo_datasets/data_geo_" + str(geo_id), sep=';', index=False)
    dataset_data[1].to_csv("geo_datasets/data_geo_" + str(geo_id) + "_response", sep=';', index=False)
